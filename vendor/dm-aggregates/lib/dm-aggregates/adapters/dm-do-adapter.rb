module DataMapper
  module Aggregates
    module DataObjectsAdapter
      ...
      chainable do
        p "CHAINABLE--"
        # This method replaces the adapter property_to_column_name
        #
        #
        def property_to_column_name(property, qualify)
          p "DM-AGGREGATE-property_to_column_name #{property.inspect}"
          case property
            when DataMapper::Query::Operator
              aggregate_field_statement(property.operator, property.target, qualify)

            when Property, DataMapper::Query::Path
              # Regular property => Super to process (DataObjectsAdapter)
              super
